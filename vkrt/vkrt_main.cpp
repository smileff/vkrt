...
//	if (!VKSucceed(vkCreateDevice(vkPhysicalDevice, &deviceCInfo, g_vkAllocator, vkDevice))) {
//		return false;
//	}
//
//	return true;
...
private:
	VKSingleQueueDeviceContext m_context;
};
...
bool VKRTApplication::Initialize(SDL_Window* sdlWin, const VkInstance& vkInst, const VkSurfaceKHR& vkSurf)
{
	...
	if (!m_context.InitializeDeviceContext(enabledLayerNames, enableExtensionNames)) {
		return false;
	}
	if (!m_context.InitializeSwapchain()) {
		return false;
	}
	...
	uint32_t queueFamilyIdx = m_context.GetQueueFamilyIdx();
	VkCommandPool commandPool = m_context.GetCommandPool();
	VkSurfaceFormatKHR surfFmt = m_context.GetSurfaceFormat();
	...
	SDL_Vulkan_GetDrawableSize(sdlWin, (int*)&rtWidth, (int*)&rtHeight);

	// Print device infos.
	VKPrintPhysicalDeviceName(physicalDevice);
	VKPrintPhysicalDeviceFeatures(physicalDevice);
	VKPrintPhysicalDeviceQueueFamilyProperties(physicalDevice);

	// Print the capabilities of this surface.	
	VKPrintSurfaceCapabilities(SDL_GetWindowTitle(sdlWin), physicalDevice, vkSurf);

	// Then intiailze for concrete rendering.


	// Allocate a command buffer for each swapchain image.
	std::vector<VkCommandBuffer> imageCmdBufs(swapchainImageViewCnt);
	if (swapchainImageViewCnt > 0) {
		VkCommandBufferAllocateInfo cmdBufAllocInfo = {
				.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
				.commandPool = commandPool,
				.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY,
				.commandBufferCount = (uint32_t)swapchainImageViewCnt,
		};
		VKCall(vkAllocateCommandBuffers(device, &cmdBufAllocInfo, imageCmdBufs.data()));
	}
